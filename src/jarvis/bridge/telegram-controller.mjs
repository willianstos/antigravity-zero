...
bot.on('text', async (ctx) => {
    ...
    if (intent) {
        ...
        // No intent match — show help or use LLM?
        ctx.reply(
            '🤖 **Jarvis Sovereign Mode**\n\n' +
            'Não detectei comando direto. Tente:\n' +
            '• `gemini: Sua pergunta` (Sessão Web)\n' +
            '• `search: Sua busca` (Perplexity)\n' +
            '• `recall: Assunto` (Qdrant Memory)\n' +
            '• `screenshot`, `ler tela`, `status`'
        );
    }
});
